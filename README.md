...
- **Default Provider Routing**: Force mode wraps the `claude` binary so all contexts (OMC, tmux, scripts) route through your chosen provider
- **Build System**: Modular source with automated release pipeline (`build.sh --release patch|minor|major`)
...
```bash
ccswitch --version      # Should print: CCSwitch v1.x.x
...
├── ccswitch.sh           # Built single-file distribution (auto-generated by build.sh)
├── build.sh              # Build & release automation
...
```bash
# Build single-file distribution from modules
bash build.sh

# Release with automatic version bump
bash build.sh --release patch    # 1.5.5 -> 1.5.6
bash build.sh --release minor    # 1.5.5 -> 1.6.0
bash build.sh --release major    # 1.5.5 -> 2.0.0
```
...
- **자동 업데이트**: 자체 업데이트 기능
- **기본 프로바이더 라우팅**: Force 모드로 `claude` 바이너리를 래핑하여 모든 컨텍스트(OMC, tmux, 스크립트)에서 선택한 프로바이더로 라우팅
- **빌드 시스템**: 모듈러 소스와 자동화된 릴리스 파이프라인 (`build.sh --release patch|minor|major`)
- **마이그레이션 지원**: Clother에서 자동 마이그레이션
...
```bash
ccswitch --version      # 출력: CCSwitch v1.x.x
...
├── ccswitch.sh           # 빌드된 단일 배포 파일 (build.sh로 자동 생성)
├── build.sh              # 빌드 및 릴리스 자동화
├── models.json           # 프로바이더 모델 레지스트리
├── lib/
...
- **환경 변수**: `CLOTHER_CONFIG_DIR`, `CLOTHER_DATA_DIR`, `CLOTHER_CACHE_DIR`, `CLOTHER_BIN`은 지원 중단(deprecated) 경고와 함께 계속 인식됨
